...
def decompose(data, data_cols=None, kind='ICA', n_components=None):
    decompositor = None
    if kind == 'ICA':
        decompositor = FastICA(n_components=n_components)
    elif kind == 'PCA':
        decompositor = PCA(n_components=n_components)
    elif kind == 'Kernel':
        decompositor = KernelPCA(n_components=n_components)
